...
ggplot(aes(x = ClosedDate), data = loans) +
  #these data are of the type POSIXct and thus the unit it's plotting is
  #actually seconds. Thus, 1 year = 60 sec * 60 min * 24 hr * 365.25 days
  geom_histogram(binwidth = (60 * 60 * 24 * 365.25)/12) +
  scale_x_datetime(date_labels = "%m-%Y", date_breaks = "6 months") +
...
```{r Facets: Term}
ggplot(aes(x = CreditScoreMidpt, y = MonthlyLoanPayment), data = loans) +
  geom_jitter(height = 0, alpha = 1/20) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 1)) +
  facet_wrap(~Term) +
  scale_x_continuous(limits = c(350,800), breaks = seq(350,800,40)) +
  ggtitle("Monthly Payments vs. Credit Score", subtitle = "Faceted by Loan Term")
```
...
```{r Facets: Term, Bubbles: Principal}
summary(loans$LoanOriginalAmount)

ggplot(aes(x = CreditScoreMidpt, y = MonthlyLoanPayment), data = loans) +
  #geom_jitter(height = 0, alpha = 1/50, aes(size = LoanOriginalAmount)) +
  geom_point(alpha = 1/50, aes(size = LoanOriginalAmount)) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 1)) +
  facet_wrap(~Term, ncol = 1) +
  scale_x_continuous(limits = c(420,800), breaks = seq(420,800,40)) +
  #Keep legend alpha = 1.0
  guides(colour = guide_legend(override.aes = list(alpha = 1))) +
  ggtitle("Monthly Payments vs. Credit Score", 
          subtitle = "Faceted by Loan Term, Bubbles Sized by Principal")
```
...
```{r Rates and Monthly Payments: Faceted by Listing Category}
ggplot(aes(x = BorrowerRate, y = MonthlyLoanPayment), data = loans) +
  geom_point(alpha = 1/100)
...
```{r Rates and Monthly Payments: Faceted by Term}
ggplot(aes(x = BorrowerRate, y = MonthlyLoanPayment), data = loans) +
  geom_point(alpha = 1/100, aes(color = LoanOriginalAmount))# +
 # facet_wrap(~Term, ncol = 1)
```
...
```{r Binning by Principal and Faceting}
loans$PrincipalBins <- cut(loans$LoanOriginalAmount, 
                           breaks = seq(0,36000, 6000),
                           #breaks = c(0, 6000, 12000, 18000, 24000, 35000),
                           right = TRUE, ordered_result = TRUE)

ggplot(aes(x = BorrowerRate, y = MonthlyLoanPayment), data = loans) +
  geom_point(alpha = 1/100, aes(color = LoanOriginalAmount)) +
  coord_cartesian(ylim = c(0,1250)) +
  facet_wrap(~PrincipalBins) +
  geom_smooth(method = "lm", color = "red")
```
...
```{r Rates and Statuses and Credit Scores - Oh My!}
loans$CreditScoreBins <- cut(loans$CreditScoreMidpt, 
                             breaks = c(0, 500, 650, 700, 750, 800, 850), 
                             right = TRUE,
                             ordered_result = TRUE)

extremeStatuses <- subset(loans, (LoanStatus >= "Current" | 
                            LoanStatus <= "Defaulted") &
                            !is.na(CreditScoreBins))

ggplot(aes(x = LoanStatus, y = BorrowerRate), data = extremeStatuses) +
  geom_violin(draw_quantiles = c(0.25, 0.75), scale = "count", 
              aes(fill = LoanStatus)) +
  #Add in a dot for median value
  stat_summary(fun.y="median", geom="point") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1)) +
  geom_hline(yintercept = 0.25, color = "blue", linetype = "dashed") +
  facet_wrap(~CreditScoreBins) +
...
```{r Plot_One}
p1_1 <- ggplot(aes(x = BorrowerState), data = loans) +
  geom_bar(fill = "grey") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.4)) +
  geom_vline(xintercept = c(9, 12, 16, 22, 8, 39), color = "red") +
  ylab("Number of Loans") +
  xlab("Borrower's State of Residence") +
  ggtitle("Number of Loans in Each U.S. State")

p1_2 <- ggplot(aes(x = BorrowerState), data = loans) +
  geom_bar(aes(weight = 1/Population), fill = "black") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.4)) +
  geom_vline(xintercept = c(9, 12, 16, 22, 8, 39), color = "red") +
  ylab("Number of Loans Per Capita") +
  xlab("Borrower's State of Residence") +
  ggtitle("Number of Loans in Each U.S. State",
          subtitle = "Scaled by Each State's Population")
...
ggplot(aes(x = LoanStatus, y = BorrowerRate), data = extremeStatuses) +
  ...
  geom_hline(yintercept = 0.25, color = "blue", linetype = "dashed") +
  facet_wrap(~CreditScoreBins) +
  annotate("rect", xmin = 0, xmax = 2.5, ymin = -Inf, ymax = Inf, 
           fill = "red", alpha = 0.1) +
  annotate("rect", xmin = 2.5, xmax = 5.5, ymin = -Inf, ymax = Inf, 
           fill = "green", alpha = 0.1) +
  ylab("Loan Interest Rate") +
  xlab("Loan Statuses") +
  ggtitle("Distribution of Interest Rates Over Different Loan Statuses",
          subtitle = "Faceted by Borrowers' Credit Score Ranges")
```
...
ggplot(aes(x = CreditScoreMidpt, y = MonthlyLoanPayment), data = loans) +
  geom_point(alpha = 1/50,
             aes(size = LoanOriginalAmount, color = ListingCreationDate)) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 1)) +
  facet_wrap(~Term, ncol = 1) +
  scale_x_continuous(limits = c(420,800), breaks = seq(420,800,40)) +
  #Setup the point size legend to have alpha = 1 and new title
  guides(size = guide_legend("Loan Principal Amount", 
                             override.aes = list(alpha = 1),
                             reverse = TRUE)) +
  #Setup the color legend to have alpha = 1 and new title
  guides(colour = guide_legend("Loan Creation Date", 
                               override.aes = list(alpha = 1),
                             reverse = TRUE)) +
  ylab("Monthly Loan Payment ($)") +
  xlab("Borrower's Credit Score") +
  ggtitle("Monthly Payments vs. Credit Score", 
          subtitle = "Faceted by Loan Term Length in Months")
```
...
I'd also like to point out here how cool it is that this plot covers **5 dimensions**: 
